...
class Solver(object):
    ...
    def __init__(self, vocab_size, batch_size, pre_gen_epochs, pre_dis_epochs, gan_epochs, generate_sum, sequence_len,
                 lr, real_file, fake_file, eval_file, update_rate):
        ...
        self.target_lstm = target_lstm.TargetLSTM(vocab_size, GenParams.emb_dim, GenParams.hidden_dim,
    ...
    def eval_epoch(self, model, data_loader, criterion):
        ...
        total_words = 0.
        for i, (data, target) in enumerate(data_loader):
            x, y = util.to_var(data, volatile=True), util.to_var(target,
                                                                 volatile=True)  # x: (None, sequence_len + 1), y: (None, sequence_len + 1). Should use volatile if no backward operation
    ...
    def train_gan(self, backend):
        ...
        gen_optim, dis_optim = torch.optim.Adam(self.generator.parameters(), self.lr), torch.optim.Adam(
        ...
        for epoch in range(self.gan_epochs):
            ...
            if epoch % 10 == 0:
                util.generate_samples(self.generator, self.batch_size, self.sequence_len, self.generate_sum,
                                      self.eval_file)
                eval_data = GenData(self.eval_file)
                eval_data_loader = DataLoader(eval_data, batch_size=self.batch_size, shuffle=True, num_workers=8)
                loss = self.eval_epoch(self.target_lstm, eval_data_loader, gen_criterion)
